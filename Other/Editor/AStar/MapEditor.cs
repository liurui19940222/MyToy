...
public class MapEditor : Editor
{
    ...
    static void OnSceneGUI(SceneView sceneView)
    {
        ...
        if (m_path != null && m_path.Count > 0)
        {
            ...
            {
                DrawCube(m_path[i], Quaternion.identity, size, Color.Lerp(map.m_GridCustom.m_EndPointColor, map.m_GridCustom.m_StartPointColor, i / (float)m_path.Count));
            }
        }
    ...
    {
        m_path = map.FindPath(new IPoint((int)m_from.x, (int)m_from.y), new IPoint((int)m_to.x, (int)m_to.y));
    }
}
