...
public class FSMEditor : EditorWindow
{
    ...
    //检查所有无效的Transition指向并删除
    public void CheckAllUselessTransition(int hintStateId)
    {
        foreach (FSMStateWindow window in m_StateWindows.Values)
        {
            window.CheckUselessTransition(hintStateId);
        }
    }
...
public class FSMStateWindow
{
    ...
    public void CheckUselessTransition(int hintStateId)
    {
        FSMStateAction state = m_Controller.GetState(m_StateId) as FSMStateAction;
        if (state == null)
            return;
        ...
        {
            if (state.Transitions[i].TargetStateId == hintStateId)
            {
                state.Transitions.RemoveAt(i--);
            }
        }
        if (count != state.Transitions.Count)
        {
    ...
    void OnContextMenu(int index)
    {
        ...
        else if (index == 2)
        {
            ...
            m_Editor.CheckAllUselessTransition(m_StateId);
        }
    }
