...
public class XboxInput : IInputDevice {
    ...
    public override Vector2 GetAxis(EJoystick joystick)
    {
        ...
        {
            m_Axis.x = Input.GetAxis(InputConst.XBOX_KEYWORD_LEFT_JOY_H);
            m_Axis.y = Input.GetAxis(InputConst.XBOX_KEYWORD_LEFT_JOY_V);
        }
        else if (joystick == EJoystick.RIGHT)
        {
            m_Axis.x = Input.GetAxis(InputConst.XBOX_KEYWORD_RIGHT_JOY_H);
            m_Axis.y = Input.GetAxis(InputConst.XBOX_KEYWORD_RIGHT_JOY_V);
        }
        return m_Axis;
    }
    ...
    public Vector2 GetDPadAxis()
    {
        ...
        axis.x = Input.GetAxis(InputConst.XBOX_KEYWORD_DPAD_H);
        axis.y = Input.GetAxis(InputConst.XBOX_KEYWORD_DPAD_V);
        return axis;
    }
    ...
    private void TestTrigger(EInputWord trigger)
    {
        ...
        {
            value = Input.GetAxis(InputConst.XBOX_KEYWORD_LT) != 0 ? 1 : 0;
            lastValue = m_LastLTrigger;
            m_LastLTrigger = value;
        }
        else if (trigger == EInputWord.RT)
        {
            value = Input.GetAxis(InputConst.XBOX_KEYWORD_RT) != 0 ? 1 : 0;
