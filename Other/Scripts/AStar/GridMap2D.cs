 ...
 * **/
[CreateAssetMenu(fileName = "map_", menuName = "配置/地图")]
public class GridMap2D : ScriptableObject
{
    ...
    public List<IPoint> FindPath(Vector3 from, Vector3 to)
    {
        IPoint f = WorldSpaceToCellPos(from);
        IPoint t = WorldSpaceToCellPos(to);
        return _AStar.FindPath(f.X, f.Y, t.X, t.Y, 1);
    }

    public List<IPoint> FindPath(IPoint from, IPoint to)
    {
        return _AStar.FindPath(from.X, from.Y, to.X, to.Y, 1);
    }
